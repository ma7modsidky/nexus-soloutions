...
import { getJobs, getJobBySlug } from '../../utils/wordpress';
                ...
                <div className="space-y-4">
                  {recentPosts.map(recentPost => (
                    <div 
                      ...
                      onClick={() => router.push(`/careers/${recentPost.slug}`)}
                    >
...
export async function getStaticProps({ params }) {
  const post = await getJobBySlug(params.slug);
  const recentPosts = await getJobs('?per_page=4&_embed');
